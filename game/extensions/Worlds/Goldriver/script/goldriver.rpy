...
label ow_goldriver_enter:
    scene black

    'You leaving the Mists, entering an Outer World.'

label ow_goldriver_gt:
    show bg gt
    'There is a small river nearby, streaming down from the rocky hills.'

    menu:
        'Go to the river':
            call ow_goldriver_river

        'Return to the Mists':
            'You leaving the Gold River Outer World'
    return

label ow_goldriver_river:
    show bg river
    'You moved to the river'
    "It's rocky bottom is sparking with gold."

    menu:
        'Investigate the river' if ingot:
            call ow_goldriver_ingot

        'Extract golden send' if not ingot:
            call ow_goldriver_extraction

        'To the Edge of Mists':
            call ow_goldriver_gt

    return

label ow_goldriver_ingot:
